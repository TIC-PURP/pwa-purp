// src/lib/database.ts
// CouchDB + PouchDB (offline-first) y CRUD de usuarios con write-through
...
/** Carga PouchDB y plugin find (solo cliente) */
async function ensurePouch() {
  if (!isClient || PouchDB) return;
  const mod = (await import("pouchdb")).default as any;
...
function toUserDocId(input: string) {
  if (!input) return "user:_";
  if (input.includes(":")) return input;     // ya trae partición
...
function buildUserDocFromData(data: any) {
  const key = slug(data?.email || data?.name || data?.id || "");
  const _id = key ? `user:${key}` : (data?._id || `user:${Date.now()}`);
  const now = new Date().toISOString();
  return {
    _id,
    type: "user",
    id: `user_${key || Date.now()}`,
    name: data?.name ?? "",
    email: data?.email ?? "",
...
/** Logout online de la cookie de /_session */
...
export async function startSync() {
  ...
  syncHandler
    .on("change", (i: any) => {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("purp-sync", {
            detail: { type: "change", direction: i.direction },
          })
        );
      } catch {}
    })
    ...
    .on("active", () => {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("purp-sync", { detail: { type: "active" } })
        );
      } catch {}
    })
    .on("error", (e: any) => {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("purp-sync", {
            detail: { type: "error", error: String(e?.message || e) },
          })
        );
      } catch {}
    });
...
export async function authenticateUser(identifier: string, password: string) {
  ...
  if (identifier) {
    candidates.push(`user_${identifier.toLowerCase()}`);
    candidates.push(`user:${slug(identifier)}`);
  }

  // Índices útiles
  try {
    await localDB.createIndex({ index: { fields: ["email"] }, name: "idx-email" });
  } catch {}
  try {
    await localDB.createIndex({ index: { fields: ["id"] }, name: "idx-id" });
  } catch {}

  // 1) Por _id (dos variantes)
  for (const cand of candidates as string[]) {
    try {
      const byId = await localDB.get(cand);
      if (byId?.password === password) return byId;
    } catch {}
  }

  // 2) Mango por email
  try {
    const r = await localDB.find({ selector: { email: identifier } });
    const d = r.docs?.[0];
    if (d && d.password === password) return d;
  } catch {}

  // 3) Fallback por id (string exacto)
  for (const cand of candidates as string[]) {
    const r = await localDB.find({ selector: { id: cand } }).catch(() => null);
    const d = r?.docs?.[0];
    if (d && d.password === password) return d;
  }
...
/** Guarda/actualiza usuario SOLO en Pouch local (para refrescar sesión local) */
export async function guardarUsuarioOffline(user: any) {
  ...
  try {
    ...
    (toSave as any)._rev = existing._rev;
...
/** Asegura índices Mango necesarios */
...
async function putUserSmart(doc: any) {
  ...
  if (isOnline && remoteDB) {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("purp-write", { detail: { path: "remote", id: doc._id } })
        );
      } catch {}
      return doc;
    } catch (e: any) {
  ...
  try {
    window.dispatchEvent(
      new CustomEvent("purp-write", { detail: { path: "local", id: doc._id } })
    );
  } catch {}
  return doc;
}
...
export async function updateUser(idOrPatch: any, maybePatch?: any) {
  ...
  // Si existe, usa su _rev; si no, no lo pongas (insertará nuevo)
  const existing: any = await localDB.get(_id).catch(() => null);
  const updated: any = {
    ...(existing || {}),
    ...patch,
    _id,
...
export async function deleteUserSmart(idOrKey: string): Promise<boolean> {
  ...
  if (isOnline && remoteDB) {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("purp-write", {
            detail: { path: "remote", id: _id, op: "delete" },
          })
        );
      } catch {}
      return true;
  ...
  if (localDoc) {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("purp-write", { detail: { path: "local", id: _id, op: "delete" } })
      );
    } catch {}
    return true;
  }
...
/** Busca por email (varias estrategias) */
export async function findUserByEmail(email: string) {
  await openDatabases();
  const idByEmail = `user_${email}`;
  try {
    return await localDB.get(idByEmail);
  } catch {}
  try {
    await ensureUserIndexes();
    const r = await localDB.find({ selector: { email }, limit: 1 });
...
export async function watchUserDocByEmail(
  ...
  const ids = [`user_${email}`, toUserDocId(email)];
  ...
  const feed = localDB
    ...
    .on("change", (ch: any) => {
      if (!ch?.doc) return;
      if (ids.includes(ch.doc._id)) onUpdate(ch.doc);
      else if (ch.doc?.email && ch.doc.email === email) onUpdate(ch.doc);
    })
    .on("error", (err: any) => {
