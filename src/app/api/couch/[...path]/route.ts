...
async function proxy(req: NextRequest, path: string[]) {
  try {
    ...
    if (!targetBase) return NextResponse.json({ ok:false, error:"Missing COUCH_HOST" }, { status:500 });
    ...
    const setCookie = res.headers.get("set-cookie"); if (setCookie) nextRes.headers.set("set-cookie", setCookie);
    const resCT = res.headers.get("content-type"); if (resCT) nextRes.headers.set("content-type", resCT);
    nextRes.headers.set("Cache-Control", "no-store");
    return nextRes;
  } catch (e:any) {
    return NextResponse.json({ ok:false, error:"Proxy error", message: e?.message ?? String(e) }, { status:500 });
  }
}
