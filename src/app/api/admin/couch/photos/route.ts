// === /app/api/photos/route.ts (Next.js App Router) ===
// Endpoint admin para crear/actualizar (PUT) y eliminar (DELETE) fotos en CouchDB
// Usa credenciales admin (Basic Auth) desde variables de entorno
...
function getCouchDbName(): string {
  const direct = process.env.COUCHDB_DB_PHOTOS || process.env.COUCHDB_DB;
...
export type AdminPhotoPutPayload = {
  // Documento completo que quieres escribir en CouchDB
  doc: Record<string, any> & { _id: string; _rev?: string };
  // Adjuntos en formato CouchDB _attachments (base64 embebido)
  attachments?: {
    [name: string]: { content_type: string; data: string };
  };
  // Si quieres forzar _attachments aquí en el servidor (sobrescribe doc._attachments)
  forceAttachments?: boolean;
};

export async function PUT(req: NextRequest) {
  try {
    const body = (await req.json()) as AdminPhotoPutPayload;
    ...
    const res = await fetch(couchDocUrl(payload._id), {
      ...
      // Nota: CouchDB permite PUT con documento completo, incluyendo _attachments base64
    });

    const data = await res.json().catch(() => ({}));
    console.log("[admin/photos] PUT respuesta", res.status, data);
    if (!res.ok) {
...
export type AdminPhotoDeletePayload = {
  _id: string;
  _rev?: string; // si no viene, el server intentará obtenerlo
};
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log("[admin/photos] GET", { id, target: couchDocUrl(id) });
    const res = await fetch(couchDocUrl(id), { headers: buildHeaders() });
    const data = await res.json().catch(() => ({}));
    console.log("[admin/photos] GET respuesta", res.status, data);
    if (!res.ok) {
...
export async function DELETE(req: NextRequest) {
  try {
    const body = (await req.json()) as AdminPhotoDeletePayload;
    if (!body?._id) {
      return new Response(JSON.stringify({ error: "_id requerido" }), { status: 400 });
    }

    console.log("[admin/photos] DELETE", { id: body._id, suppliedRev: body._rev });
    ...
    if (!rev) {
      ...
      if (!rev) {
        // fallback GET (por si ETag no viene)
    ...
    console.log("[admin/photos] DELETE respuesta", del.status, data);
    if (!del.ok) {
