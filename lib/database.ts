...
// ---------- UTIL: crea un manager por defecto si no existe ----------
export async function initializeDefaultUsers() {
  const email = process.env.NEXT_PUBLIC_DEFAULT_MANAGER_EMAIL || 'manager@example.com'
  const name = process.env.NEXT_PUBLIC_DEFAULT_MANAGER_NAME || 'Manager'
  const id = 'default-manager'
  const res = await (localDB as any).find({ selector: { type: 'user', email } })
  if (res.docs.length === 0) {
    ...
    const doc: User = {
      ...
      email,
      role: 'manager',
      permissions: ['users_read', 'users_write', 'users_delete', 'app_access'],
      isActive: true,
      createdAt: now,
      updatedAt: now,
    }
